                                ...
                                async def load_config():
                                    try:
                                        ...
                                        if uploaded_file.filename.endswith(".json"):
                                            ...
                                            model_words = ["gpt-3.5-turbo", "gpt-3.5", "gpt-4", "gpt-4o", "gpt-3.5-turbo gpt-4", "gpt-4-1106-preview", "gpt-4-0613", "gpt-4-0314", "gpt-3.5-turbo-16k-0613", "gpt-3.5-turbo-16k", "gpt-3.5-turbo-1106", "gpt-3.5-turbo-0613", "gpt-3.5-turbo-0301"]

                                            if session.model not in model_words:
                            ...
                            if utilities == "extend-all-channels":
                                async def dispatch_session(shared_sessions):
                                    try:
                                        ...
                                        # Split session_channels_str into multiple fields if it exceeds the limit
...
@slash.slash(name="advanced-options", 
    description="Set advanced options for gpt debates or generating data",
    options=[
        create_option(
            name="bot_share",
            description="Share messaging with another bot",
            option_type=3,
            required=False,
        ),
        create_option(
        ...
        try:
            ...
            while self.chat_session:
                ...
                if (message.author.id == self.ctx.author.id or not has_session) and (message.channel.id == self.ctx.channel.id or message.channel.id in self.server_share):
                    ...
                    if message.author.bot and str(message.author.id) not in [entry.strip('<@!>') for entry in self.bot_list]:
                        continue
                    ...
                    if (str(message.author.id) == user_id or f"<@{message.author.id}>" in self.user_share.keys() or any(role in self.role_share.keys() for role in message.author.roles)):
                        ...
                        if self.chat_logger == True and str(message.author.id) in [entry.strip('<@!>') for entry in self.bot_list]:
                            # Append conversation log entry to the list
                            # we should gather certain bits like the role etc and add also
                            self.generated_chat_log.append(f"{self.tagname_two}: {message.content}")

                        async def rev_engine(self, user_id, dm_channel, message):

                            if message.author.bot and str(message.author.id) in [entry.strip('<@!>') for entry in self.bot_list]:

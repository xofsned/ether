"""
Written by Ned
Edition: v29.4, 9.6.2024   
Replace every instance of 1147188428048437358 with your bot ID    
"""
...
async def on_ready():
    ...
    ether_session = EthersSession(bot)
    asyncio.create_task(ether_session.start())
...
async def ether_admin():
    global schedule, bugs, admin_channels, blacklist_dict, homeserv, admin_id, ether_ai, ether_temperature, ether_model, ether_tokens, ether_contextLength, ether_api_base, ether_heartbeat, ether_user_message_counts, ether_lock, ether_initialized, ether_prompt
        ...
        if check_admin_author_id(message) and check_command_keyword(message):
            ...
            if command == "banuser":
                ...
                if action == 'schedule':
                    ...
                    ether_ai = message_text
                    await message.channel.send(f"Ether's AI Status has been updated to: {message_text}")

                elif action == 'prompt':
                    ether_prompt["content"] = message_text
                    await message.channel.send(f"Ether prompt has been updated to: {message_text}")

                elif action == 'temperature':
                    try:
                        ether_temperature = float(message_text)
                        await message.channel.send(f"Ether's temperature has been updated to: {ether_temperature}")
                    except ValueError:
                        await message.channel.send("Invalid value for ether_temperature. Please provide a valid number.")

                elif action == 'model':
                    ether_model = message_text
                    await message.channel.send(f"Ether's model has been updated to: {ether_model}")

                elif action == 'tokens':
                    try:
                        ether_tokens = int(message_text)
                        await message.channel.send(f"Ether's tokens have been updated to: {ether_tokens}")
                    except ValueError:
                        await message.channel.send("Invalid value for ether_tokens. Please provide a valid integer.")

                elif action == 'context':
                    try:
                        ether_contextLength = int(message_text)
                        await message.channel.send(f"Ether's context length has been updated to: {ether_contextLength}")
                    except ValueError:
                        await message.channel.send("Invalid value for ether_contextLength. Please provide a valid integer.")

                elif action == 'api_base':
                    ether_api_base = message_text
                    await message.channel.send(f"Ether's API base has been updated to: {ether_api_base}")
...
async def manager(ctx, session_management=None, session_name=None, extend_session=None, save_load=None, utilities=None, exit_session_number=None, exit=None, api_base=None):
    ...
    options = (session_management, session_name, extend_session, save_load, utilities, exit_session_number, exit, api_base)
                ...
                if session_list:
                    ...
                    if session is not None:
                        ...
                        if utilities is not None:
                            ...
                            if utilities == "extend-all-channels":
...
async def advanced_options(ctx, bot_share=None, bot_share_delay=None, chat_logger=None, download_log=None, clear_log=None, set_chat_user_1=None, set_chat_user_2=None):
    global user_sessions, schedule, bot_login_time, bugs, homeserv
            ...
            with user_sessions_fetch_lock:
                ...
                if session_list:
                    ...
                    if session is not None:
                        ...
                        options = (bot_share, bot_share_delay, download_log, clear_log, chat_logger, set_chat_user_1, set_chat_user_2)
                        if all(option is None for option in options):
                            async def get_stats_task(ctx, session):
                                try:
...
async def get_ether(ctx, status_message):
    global homeserv, ether_ai, ether_ai_model
    async def get_ether_task(homeserv):
        try:
            ...
            embed.add_field(name="- Ether AI", value=f" * Status: {ether_ai}\n- Model: {ether_ai_model}", inline=False)
            embed.add_field(name="- LM Studio API", value=f" * Configured for V0.3.1", inline=False)
            if feed is not None:
    ...
    await asyncio.create_task(get_ether_task(homeserv))
            ...
            async def hello_ether():
                ...
                try:
                    ...
                    async with user_sessions_lock:
                        has_session = any(
                            key.split("-")[0] == str(message.author.id)
                    ...
                    if not has_session and not shared_session:
                        ...
                        async for msg in message.channel.history(limit=10):
                            role = "assistant" if msg.author.id == 1147188428048437358 else "user"
                            if msg.content.strip():  # Ensure the message content is not empty
                                content = msg.content.replace(f'<@{1147188428048437358}>', '').strip()
                        ...
                        if message.content.strip():  # Ensure the current message content is not empty
                            content = message.content.replace(f'<@{1147188428048437358}>', '').strip()
                ...
                if (message.author.id == self.ctx.author.id or not has_session) and (message.channel.id == self.ctx.channel.id or message.channel.id in self.server_share):
                    ...
                    if (str(message.author.id) == user_id or f"<@{message.author.id}>" in self.user_share.keys() or any(role in self.role_share.keys() for role in message.author.roles)):
                        ...
                        async def rev_engine(self, user_id, dm_channel, message):
                            ...
                            if self.api_base == "LMStudio" and re.sub(r'<@1147188428048437358>', '', message.content).strip().lower().startswith(('embed', 'draw', 'variate', 'set', 'exit')):
                            ...
                            if re.sub(r'<@1147188428048437358>', '', message.content).strip().lower().startswith('embed') == False and (0 < len(message.attachments) <= 2) and ("Fast_Embed" in user_access or "Visual" in user_access):
                            ...
                            if re.sub(r'<@1147188428048437358>', '', message.content).strip().lower().startswith('embed') and "Full_Embed" in user_access:
                            ...
                            if re.sub(r'<@1147188428048437358>', '', message.content).strip().lower().startswith('set') and "Set" in user_access:
                            ...
                            if "Chat" in user_access:
                                ...
                                if self.learning_session == True and self.session_set == False:
                                    if re.sub(r'<@1147188428048437358>', '', message.content).strip().lower().startswith('exit') and "Full_Embed" in user_access:
                                    ...
                                    if self.session_set == False:
                                        ...
                                        if self.context == 0:
                                            ...
                                            if self.alternating_context == "True":
                                                ...
                                                def ensure_alternating_roles(conversationLog):
                                                    user_messages = [msg for msg in conversationLog if msg["role"] == "user"]
                                                    assistant_messages = [msg for msg in conversationLog if msg["role"] == "assistant"]

                                                    if len(user_messages) > len(assistant_messages):
                                                        combined_user_content = " ".join(msg["content"] for msg in user_messages)
                                                        user_messages = [{"role": "user", "content": combined_user_content}]

                                                    if len(assistant_messages) > len(user_messages):
                                                        combined_assistant_content = " ".join(msg["content"] for msg in assistant_messages)
                                                        assistant_messages = [{"role": "assistant", "content": combined_assistant_content}]
                                                    ...
                                                    fixed_log = [conversationLog[0]]  
                                                    for i in range(min_length):
                                                        fixed_log.append(user_messages[i])
                                                        fixed_log.append(assistant_messages[i])

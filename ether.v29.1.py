...
DESIGNATED_CHANNEL_ID = 1098206996404109392
ROLE_ID_SESSIONS = 1206626445934469150
ROLE_ID_SESSIONS_EXTENDING = 1097514204203864084
ROLE_ID_DATA = 1206717883225473034 
ROLE_ID_LIMITED = 1093894096239722507 
ROLE_ID_DEV = 1094745782579646605 
ROLE_IDS = [1097514204203864084, 1093894096239722507] 
admin_channels = (907301373387898954, 1098206996404109392, 1171869093289279518) 
# <]

# ether & beep boop stuff [>
bot = commands.Bot(command_prefix='/', intents=intents)
slash = SlashCommand(bot, sync_commands=True) # sync commands true will sync for guild in guilds and in direct message
schedule = 'No Scheduled Outages' # alter this by administrative command
bugs = 'No Known Active Errors' # depreciated and replaced with gitbook fetch in /ether
uptime = '' # helps track uptime
homeserv = "" # resource server as changable variable in admin commands - add your server invite link and alter it by command if link ever expires or errors
bot_login_time = None # initializing this to store bot login time for calculating overall uptime
presence_flag = True # this just formats the initial online message
# <]
...
# << --------- NOTES ------------>>
# The initial entry of the class is in /manager function line ~1325
# We generate a user key to bind to the memory address of the class instance
# All sessions are managed by the global user_sessions and active_sessions object, accessed with locks
# Global user_session is required for accessing / storing session keys, allowing users to alter their class variables by key
# << ------------- NOTES ---------------- >>

# << -------------- STATUS ----------- >>
# working on SQL for embedding in file processing, needing more unique identifiers in the data conversion
# current conversion process working with sqlparse, though data is not meaningful enough for ChatGPT to interpret with accuracy
# recent file type additions - pcap, json, xml, ocsf should receive more testing but are seemingly functional
# << -------------- STATUS ----------- >>

# << ------------- STRUCTURE ----------- >>
# when you collapse all:
# 1: Ether functions, events, tasks
# 2: Slash command families
# 3: UserSession class instance
# 4: Info and terms functions
# << ------------- STRUCTURE ----------- >>

# Objects for creating terms and blacklist tables
# Ether has the ability to ban guilds, leave builds, ban users, block users, and manages terms
# Loading pre-reqs into global dictionaries [>>>

# new list -----> 4/10/2024
# Maybe set keyword embed reply embed to non hidden to match closing embed
# Explore ways to allow for better interpretation of converted tables such as xlsx
# Add api structure for local ml hehe

# test:: -- >>
# Setting role or prompt for specific users
# Enable / disable chat context cleaner
# Add command for adding endpoint for api_base
...
# This was a nice perk as a public bot to display server count, but did not seem to be accurate, possible bug
@tasks.loop(hours=24)
...
async def ether_admin():
    ...
    def check_command_keyword(message):
        return re.sub(r'<@1147188428048437358>', '', message.content).strip().startswith(('banuser', 'banguild', 'unban', 'list', 'setstatus', 'logout', 'leave', 'setpfp'))
...
# Function to return role status and assign the status to variable
            ...
            try:
                ...
                if len(session_list) > 0:
                    ...
                    for chunk in chunks:
                        ...
                        embed.remove_field(-1)  # Remove the last field for the next iteration
                else:
                ...
                if session_list:
                    ...
                    if session is not None:
                            ...
                            async def add_session_prompt():
                                try:
                                    if author_id in session.role["user_id"]:
                                        ...
                                        session.role["prompt"][index] = add_prompt  # Update the prompt at the corresponding index
                                    print(session.role)
                                except Exception as e:
                        ...
                        if user_prompt is not None or user_role is not None:
                            ...
                            # Inside the generate_new_role_entry function
                        ...
                        if api_base is not None:
                            print("api base is not None")
                            async def set_api_base():
                                try:
                                    print(api_base)
                                    if api_base == "default":
                                        session.api_base = session.api_base_default
                                        print(f"session base changed: {api_base}")
                                    else:
                            ...
                            async def add_user_share_chat():
                                try:
                                    ...
                                    if user_share is not None or role_share is not None:
                                        ...
                                        # Maybe do an early check for conflicts in session.server_share and shared_sessions
                                        # Share accessibility overlap in session when user_share, role_share, not inclusive of session.server_share
                                        # Issue in pre-existing session.server_share instances, then share does not handle conflicts in extensions     
...
class UserSession:
    ...
    def __init__(self, ctx, session_number=None, roleStatus=None):
        ...
        self.ctx = ctx
        self.bot = bot
        self.ether_id = 1147188428048437358
        # <]
    ...
    async def exit_session_instance(self):
        ...
        print(f"servershare entering: {self.server_share}")
            ...
            with user_sessions_fetch_lock:
                with active_sessions_fetch_lock:
                    with shared_sessions_lock:
                        ...
                        print(f"self.server_share: {self.server_share}")
                        print(f"shared_sessions: {shared_sessions}")
                        print(f"Before removal: {shared_sessions}")
                        for channel_id in server_share_copy:
                            print(f"Checking channel_id: {channel_id}")
                            if channel_id in shared_sessions.keys():
                                print(f"Removing channel_id: {channel_id}")
                                del shared_sessions[channel_id]
                                self.server_share.remove(channel_id)
                        if channel_id in shared_sessions.keys():
                            del shared_sessions[channel_id]
                        print(f"After removal: {shared_sessions}")
                        self.server_share = []
    ...
    async def mini_bot_relay(self, instruction):  # Add 'async' here
        index = self.index
        if index is not None:
    ...
    async def chat(self, default, session_number):
        ...
        print(f"setting self.role: {self.role}")
                ...
                if (message.author.id == self.ctx.author.id or not has_session) and (message.channel.id == self.ctx.channel.id or message.channel.id in self.server_share):
                    ...
                    if (str(message.author.id) == user_id or f"<@{message.author.id}>" in self.user_share.keys() or any(role in self.role_share.keys() for role in message.author.roles)):
                        ...
                        async def rev_engine(self, user_id, dm_channel, message):
                            ...
                            message_content = re.sub(r'<@!?1147188428048437358>', '', message.content)
                            ...
                            if re.sub(r'<@1147188428048437358>', '', message.content).strip().startswith('embed') == False and (0 < len(message.attachments) <= 2) and ("Fast_Embed" in user_access or "Visual" in user_access):
                                        ...
                                        if "Fast_Embed" in user_access:
                                            async def quick_embed():
                                                try:
                                                    ...
                                                    if '<@1147188428048437358>' in instruction:
                                                        instruction = instruction.split('<@1147188428048437358>')[1].strip()
                            ...
                            if re.sub(r'<@1147188428048437358>', '', message.content).strip().startswith('embed') and "Full_Embed" in user_access:
                                        ...
                                        if message.attachments:
                                            ...
                                            if file_extension in ['txt', 'doc', 'docx', 'odt', 'rtf', 'python', 'lua', 'epub', 'c', 'cpp', 'rust', 'nim', 'csv', 'excel', 'js', 'sh', 'ps1', 'css', 'php', 'html', 'conf', 'log', 'pcap', 'ocsf', 'xml', 'json', 'sql']:
                                                ...
                                                try:
                                                    ...
                                                    user_session_dir = os.path.join(data_dir, session_folder)
                                                    print(f"session number: {self.session_number} dir: {session_folder}")
                            ...
                            if re.sub(r'<@1147188428048437358>', '', message.content).strip().startswith('set') and "Set" in user_access:
                            ...
                            if "Chat" in user_access:
                                ...
                                if self.learning_session == True:
                                    if re.sub(r'<@1147188428048437358>', '', message.content).strip().startswith('exit') and "Full_Embed" in user_access:
                                        ...
                                        if self.context == 0:
                                            ...
                                            if self.segmented_context == "True":
                                                ...
                                                if message.author.id in self.role["user_id"]:
                                                    ...
                                                    print(f"author instance prompt: {instance_prompt}")
                                                else:
                                                    if self.ctx.author.id in self.role["user_id"]:
                                                        index = self.role["user_id"].index(self.ctx.author.id)
                                                        user_role = self.role["user_role"][index]
                                                        instance_prompt = self.role["prompt"][index]
                                                        print(f"ctx author instance prompt: {instance_prompt}")
                                    ...
                                    if self.session_set == False:
                                        # Check this above, why are we not sanitizing session set?
                                        ...
                                        # remove debug later
                                        print(f"Log: {conversationLog} self.role: {self.role}")
                                    print(f"api engine: {self.api_engine} api endpoint: {self.api_base}")
                                    print(f"global shares: {self.server_share}")
                ...
                try:
                    ...
                    async with aiohttp.ClientSession() as session:
                        ...
                        if 'data' in response_json:
                            ...
                            # Send the message with only the image attachments
